...
static void
linreg_destroy(struct servo* servo)
{
...
static void
move_reference(struct linreg_servo* s, int64_t x, int64_t y)
{
    struct result* res;
    unsigned int i;
...
static void
update_reference(struct linreg_servo* s, uint64_t local_ts)
{
    double x_interval;
    int64_t y_interval;
...
static void
add_sample(struct linreg_servo* s, int64_t offset, double weight)
{
...
static void
update_size(struct linreg_servo* s)
{
    struct result* res;
    double best_err;
    int size, best_size;
    ...
    for (size = MIN_SIZE; size <= MAX_SIZE; size++) {
        ...
        if ((!best_size && res->slope) ||
            (best_err * ERR_EQUALS > res->err && res->err_updates >= ERR_INITIAL_UPDATES)) {
            best_size = size;
            best_err = res->err;
        }
    }
...
static double
linreg_sample(struct servo* servo, int64_t offset, uint64_t local_ts, double weight, enum servo_state* state)
{
    struct linreg_servo* s = container_of(servo, struct linreg_servo, servo);
    struct result* res;
    int corr_interval;
    ...
    pr_debug(
      "linreg: points %d slope %.9f intercept %.0f err %.0f", 1 << s->size, res->slope, res->intercept, res->err);
...
static void
linreg_sync_interval(struct servo* servo, double interval)
{
    struct linreg_servo* s = container_of(servo, struct linreg_servo, servo);
...
static void
linreg_reset(struct servo* servo)
{
    struct linreg_servo* s = container_of(servo, struct linreg_servo, servo);
    unsigned int i;
...
static double
linreg_rate_ratio(struct servo* servo)
{
    struct linreg_servo* s = container_of(servo, struct linreg_servo, servo);
...
static void
linreg_leap(struct servo* servo, int leap)
{
    struct linreg_servo* s = container_of(servo, struct linreg_servo, servo);
