...
static double
pi_sample(struct servo* servo, int64_t offset, uint64_t local_ts, double weight, enum servo_state* state)
{
    struct pi_servo* s = container_of(servo, struct pi_servo, servo);
    ...
    case 1:
        ...
        s->drift += (1e9 - s->drift) * (s->offset[1] - s->offset[0]) / (s->local[1] - s->local[0]);
    ...
    case 2:
         ...
         * clock startup.
         */
        if (servo->step_threshold && servo->step_threshold < llabs(offset)) {
            *state = SERVO_UNLOCKED;
            s->count = 0;
            break;
        }
...
static void
pi_sync_interval(struct servo* servo, double interval)
{
    struct pi_servo* s = container_of(servo, struct pi_servo, servo);
    ...
    pr_debug("PI servo: sync interval %.3f kp %.3f ki %.6f", interval, s->kp, s->ki);
}

static void
pi_reset(struct servo* servo)
{
    struct pi_servo* s = container_of(servo, struct pi_servo, servo);
...
pi_servo_create(struct servo_config* cfg)
{
    ...
    s->drift = cfg->intial_adj;
    s->last_freq = cfg->intial_adj;
    s->kp = 0.0;
    s->ki = 0.0;
    s->configured_pi_kp = cfg->kp;
    s->configured_pi_ki = cfg->ki;
    s->configured_pi_kp_scale = cfg->kp_scale;
    s->configured_pi_kp_exponent = cfg->kp_exponent;
    s->configured_pi_kp_norm_max = cfg->kp_norm_max;
    s->configured_pi_ki_scale = cfg->ki_scale;
    s->configured_pi_ki_exponent = cfg->ki_exponent;
    s->configured_pi_ki_norm_max = cfg->ki_norm_max;
