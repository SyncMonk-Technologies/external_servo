...
static void
mmedian_destroy(struct filter* filter)
{
...
static tmv_t
mmedian_sample(struct filter* filter, tmv_t sample)
{
    struct mmedian* m = container_of(filter, struct mmedian, filter);
    int i;
    ...
    for (i = m->cnt - 1; i > 0; i--) {
        if (tmv_cmp(m->samples[m->order[i - 1]], m->samples[m->index]) <= 0)
            break;
        m->order[i] = m->order[i - 1];
    }
    m->order[i] = m->index;
    ...
    if (m->cnt % 2)
        ...
        return tmv_div(tmv_add(m->samples[m->order[m->cnt / 2 - 1]], m->samples[m->order[m->cnt / 2]]), 2);
}

static void
mmedian_reset(struct filter* filter)
{
    struct mmedian* m = container_of(filter, struct mmedian, filter);
    m->cnt = 0;
    m->index = 0;
